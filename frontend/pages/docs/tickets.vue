<template>
  <div class="min-h-full bg-white dark:bg-neutral-950">
    <div class="docs-header border-b border-primary/40 dark:border-primary/30 py-6 sm:py-8">
      <div class="max-w-3xl mx-auto px-4 sm:px-6">
        <nav class="flex items-center gap-1.5 text-sm">
          ...
          <ChevronRight class="w-3.5 h-3.5 text-neutral-900/30" />
          <h1 class="inline text-neutral-900 font-semibold text-lg sm:text-xl">{{ $t('about.ticketsSystem.title') }}</h1>
        </nav>
      </div>
    </div>
    ...
    <div class="max-w-3xl mx-auto px-4 sm:px-6 py-8 sm:py-12 space-y-8">
      ...
      <p class="prose-section">{{ $t('about.ticketsSystem.intro') }}</p>

      <!-- Purchase flow -->
      <div>
        <h3 class="font-semibold text-neutral-900 dark:text-white mb-3">{{ $t('about.ticketsSystem.buyFlow.title') }}</h3>
        <ol class="space-y-2 text-neutral-700 dark:text-neutral-300">
          <li v-for="i in 4" :key="i" class="flex gap-3">
            ...
            <span>{{ $t(`about.ticketsSystem.buyFlow.step${i}`) }}</span>
          </li>
        </ol>
      </div>
      ...
      <div class="grid sm:grid-cols-2 gap-4">
        <div v-for="feature in features" :key="feature" class="p-4 border border-neutral-200 dark:border-neutral-800 rounded-lg">
          <div class="font-semibold text-neutral-900 dark:text-white mb-1">{{ $t(`about.ticketsSystem.${feature}.title`) }}</div>
          <div class="text-sm text-neutral-600 dark:text-neutral-400">{{ $t(`about.ticketsSystem.${feature}.text`) }}</div>
        </div>
      </div>
      ...
      <div class="p-4 bg-neutral-50 dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-700 rounded-lg">
        ...
        <p class="text-sm text-neutral-600 dark:text-neutral-400">{{ $t('about.ticketsSystem.noEscrow.text') }}</p>
      </div>
...
useHead({ title: computed(() => `${t('about.ticketsSystem.title')} — Parahub`) })
useSeoMeta({
  description: computed(() => t('docs.tickets_desc')),
  ogTitle: computed(() => `${t('about.ticketsSystem.title')} — Parahub`),
  ogDescription: computed(() => t('docs.tickets_desc')),
})
